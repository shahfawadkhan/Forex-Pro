...
const nav = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/riyal-saudi',    icon: Globe,           label: 'Riyal-Saudi' },
    ]
  },
  {
    group: 'Finance',
  ...
  return (
    <div className={`${sidebarOpen ? 'w-52' : 'w-14'} bg-[#0f2540] flex flex-col transition-all duration-200 flex-shrink-0`}>
      <div className="flex items-center justify-between px-4 py-4 border-b border-white/5">
        ...
        <button onClick={() => dispatch(toggleSidebar())} className="text-blue-300 hover:text-white transition ml-auto">
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        {nav.map(g => (
          <div key={g.group}>
            {sidebarOpen && (
              <div className="px-4 py-2 text-[9px] uppercase tracking-widest text-blue-400/50 font-medium">{g.group}</div>
            )}
      ...
      <div className="border-t border-white/5 p-3">
        {sidebarOpen ? (
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => dispatch(logout())} className="text-blue-300 hover:text-red-400 transition" title="Logout">
              <LogOut size={14}/>
            </button>
          </div>
        ) : (
          <button onClick={() => dispatch(logout())} className="text-blue-300 hover:text-red-400 transition">
            <LogOut size={16}/>
          </button>
        )}
      </div>
